...
#include <math.h>
#define rando() (double) random(RAND_MAX)/RAND_MAX
#include <string.h>
...
extern struct ast far * nuevonodo();
...
extern ast far * pila_records[32]; // pila de registros
...
double evalua(ast far *);


static long int counter1[32];
...
short push_argumentos(ast far *f1, ast far *g1, short * cantidad) {
...
void subcadena(int i, ast far * a, ast far * b, int var) {
    int aa, bb;
    char * subc;
    char * subc2;
    //subc = (127);
...
void subcadena2(int i, ast far * a, int var) {
    int aa, bb;
    char  *subc, *subc2 ;
    //subc = malloc(127);
    int k = 0; int j;
...
void izquierda(int i, ast far * a,  int var) {
    int aa, bb;
    char * subc, *subc2;
    //subc = malloc(127);
    int k = 0; int j;
...
void derecha(int i, ast far * a,  int var) {
...
ast far *nodo1(tiponodo, ast far *);
ast far *nodo2(tiponodo, ast far *, ast far *);
ast far *nodo3(tiponodo, ast far *, ast far *, ast far *);
ast far *nodo4(tiponodo, ast far *, ast far *, ast far *, ast far *);
...
ast far * nodo0(tiponodo Tipo, ast far * a) {
    ast far * p;
    p = nuevonodo();
    p->tipo = Tipo;
    p->num = 777;
...
ast far * nodo1(tiponodo Tipo, ast far * a) {
    ast far * p;
    p = nuevonodo();
    p->tipo = Tipo;
    p->num = 777;
...
ast far * nodo2(tiponodo Tipo, ast far * a, ast far * b)
 {
    ast far * p;
    p = nuevonodo();
    p->tipo = Tipo;
...
ast far * nodo3(tiponodo Tipo, ast far * a, ast far * b, ast far * c) {
    ast far * p;
    p = nuevonodo();
    p->tipo = Tipo;
    p->num = 777;
...
ast far * nodo4(tiponodo Tipo, ast far * a, ast far * b, ast far * c, ast far * d) {
    ast far * p;
    p = nuevonodo();
    p->tipo = Tipo;
    p->num = 777;
...
ast far * nodo5(tiponodo Tipo, ast far * a, ast far * b, ast far * c, ast far * d, ast far * e) {
    ast far * p;
    p = nuevonodo();
    p->tipo = Tipo;
    p->num = 777;
...
extern void ejecutar_string(ast far * p);
...
void calcular_tamanio(ast far * lista_de_campos) {
    //int tamanio = 0;
    int nnodos = 0;
...
void leer_campos(ast far * lista_de_campos, FILE * handler) {
    int nnodos = 0;
    int largo;
    int indice;
    char far * nombre;
    ...
    if (nnodos == 3) {
        ...
        if (depurar)
        printf("A-el campo se llama %s y contiene: %s\n", nombre, array_variables[indice].string );
       ...
       if (depurar)
        printf("B-el campo se llama %s y contiene: %s\n", nombre, array_variables[indice].string );
    }
}



void guardar_campos(ast far * lista_de_campos, FILE * handler) {
    int nnodos = 0;
    int largo;
    int indice;
    char far * nombre;
...
*/
		...
		/* en windows y msdos  debe ser  ,1,2  pero en linux ,1,1 */
            }
            fclose(handler2);
...
//interpc            yypush_buffer_state(yy_scan_string(input));
...
*/
		...
		if ((tipo != 'P') && (tipo!='F')) {
		    printf("Aprocedimiento no encontrado en linea: %d \n",  p->nrolinea2 );
...
*/
        ...
        {
	    ast far *pp; //se puso esto para depuracion (watch)
            int inum = 0;
            double fnum;
            pp = p;
            ...
            //printf("desde\n");
        {
            ...
            counter1[indice_ctr] = (long int) array_variables[x].numero;
            retornar = 0;
            for (counter1[indice_ctr]; counter1[indice_ctr] <= (long int) evalua(p->nodo3); counter1[indice_ctr]++) {
                //y = evalua(p->nodo3);
